...
export function RenderAgenda({ period }: Props) {
  const { agendas } = useAgendas()
  const { deleteAgenda } = useAgenda()

  const items = agendas.filter(a => a.period === period)
      ...
      {items.map(a => (
        ...
        >
          <div className="flex items-center gap-4">
            <span className="w-[48px] text-zinc-400 text-sm">
              {a.hour}
            </span>
